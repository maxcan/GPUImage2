...
gpsu() {
  echo "git push --set-upstream origin `g branch --no-color | grep '\*' | sed -e 's/^..//'`"
  git push --set-upstream origin `g branch --no-color | grep '\*' | sed -e 's/^..//'`
}
...
alias runKafka='eval "$(docker-machine env docker-kafka)" && docker run -p 2181:2181 -p 9092:9092 --env ADVERTISED_HOST=`docker-machine ip docker-kafka` --rm --env ADVERTISED_PORT=9092 spotify/kafka'
...
eval "$(docker-machine env docker-kafka)"
dks
alias dockerStopAll='docker stop $(docker ps -a -q)'
alias dockerKillAll='docker kill $(docker ps -a -q)'
alias dockerRmAll='docker rm $(docker ps -a -q)'
alias dockerRmiAll='docker rmi $(docker ps -a -q)'
alias nukeDocker='dockerKillAll ; dockerStopAll ; dockerRmAll'
mkdir -p /tmp/docker
